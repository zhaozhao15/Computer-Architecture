...
hex_to_str:
	...
	sw $s6, 4($sp)
	li $t3, 8           #t3 is counter 
	move $s6, $a0

loop_hex:
	beq $t3, $zero, end_hex_to_str    #end
	srl $t4, $a0, 28          #4 bit is a num or letter
	addiu $t3, $t3, -1        #counter--
	sltiu $t5, $t4, 10        #?<10 ?num
	beq $t5, 1, if_num        #num
	beq $t5, 0, if_not_bum    #letter

if_num:                       #num
	addiu $t4, $t4, 48        #ascii operation for num 
	j common_hex

if_not_bum:                    #letter
	addiu $t4, $t4, 87        #ascii operation for letter
	j common_hex

common_hex:
	sb $t4, 0($a1)            #write in
	addiu $a1, $a1, 1         #next position to write in
	sll $a0, $a0, 4           #next time will get next number(4 bit)
	j loop_hex                #loop

end_hex_to_str:
	li $t4, 10             #write \n
	sb $t4, 0($a1)
	addiu $a1, $a1, 1     #next position to write
	li $t4, 0             #write \0
	sb $t4, 0($a1)

...
write_machine_code:
	...
	# change this part.
	addiu $sp, $sp, -32
	sw $a2 24($sp) # address of symbol table
	sw $a3 28($sp) # of relocationtable
	sw $s6, 12($sp)
	sw $s7, 8($sp)
...
write_machine_code_find_text:
	lw $a0, 20($sp)	       #get a1 input pointer for func
	jal readline
	...
	li $s6, 0 #let s6 be the byte offset

write_machine_code_next_inst:
	# 2. Call readline() while passing in the correct arguments:
	# YOUR_INSTRUCTIONS_HERE
	lw $a0, 20($sp)  #get a1 for func below
	jal readline #$v0: the # of bytes read, or -1 if error and nothing was read     $v1: pointer to buffer with chars read
	#s6 not used
	...
	# YOUR_INSTRUCTIONS_HERE
	addi $a0 $v1 0 # inpt a0 = ptr to the begin of str
	li $a1, 16       #the base is 16
	jal parse_int #return the number (instruction)
	move $s7, $v0 #s7 store the instruction
	...
	addi $a0, $s7, 0 #  $a0 = an instruction that needs relocating
	addi $a1, $s6, 0   #  get a1 = the byte offset of the instruction in the current file
	lw $a2, 24($sp)   #  get a2 = the symbol table
	lw $a3, 28($sp)  #  get a3 = the relocation table
	jal relocate_inst
	#beq $v0 -1 write_machine_code_error
	move $s7, $v0#s7 = the new instruction

write_machine_code_to_file:
	# 6. Write the instruction into a string buffer via hex_to_str():
	# YOUR_INSTRUCTIONS_HERE 
	move $a0, $s7    #  get int to write
	la $a1, hex_buffer#  get character buffer to write into
	jal hex_to_str #return NONE
...
write_machine_code_end:
	...
	lw $s6, 12($sp)      #load back from stack
	lw $s7, 8($sp)
	lw $ra, 0($sp)
	lw $a2 24($sp) # address of symbol table
	lw $a3 28($sp) # of relocationtable
	lw $a0, 16($sp)		# sp+16 = output file ptr
	lw $a1, 20($sp)     # sp+20 = input file ptr
	addiu $sp, $sp, 32
	jr $ra

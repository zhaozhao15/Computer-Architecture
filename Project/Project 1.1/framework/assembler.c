...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  /* First, read next line into buffer */
  	...
  	current = strtok(buf, IGNORE_CHARS);
  	/* use strtok() to read next token*/
  	if ((!current) || ((strcmp(buf,"")==0)))
  	{
  		line++;
  		continue;
  	}
  	/*if the line is empty, just skip it*/
  	if (add_if_label(line, current, num_inst*4, symtbl) == 0)
  	{
  		/*if it is not a label, but an instruction*/
  		char name[20];
  		strcpy(name, current);
  		/*deep copy the instruction name*/
  		current = strtok(NULL, IGNORE_CHARS);
  		...
  		while(current != NULL)
  		{
  			...
  			before = current;
  			current = strtok(NULL, IGNORE_CHARS);
  			/*if too many argumets, error*/
  			if(num_args > MAX_ARGS) 
  			{	
                flag = -1;
                flag2 = -1;
                raise_extra_argument_error(line, before);
                break;
            }
  		}
  		...
  		if (flag2 != -1)
  		{
  			tmp = write_pass_one(output, name, args, num_args);
  			if (tmp == 0)
  			{
  				raise_instruction_error(line + 1, name, args, num_args);  
  				flag = -1;
  				flag3 = -1;
  			}
  		}
  		/*if no error occurred, lines are counted*/
  		if ((flag3 != -1) && (num_args <= MAX_ARGS))
  		{
  			num_inst = num_inst + tmp;
  		}
  		line++;
  	}
  	else
  	{
  		/*if we meet with a label*/
  		current = strtok(NULL, IGNORE_CHARS);
  		if ((!current) || ((strcmp(buf,"")==0)))
  		{
  			line++;
  		...
  		/*then read the instruction name*/
  		char name[20];
  		strcpy(name, current);
  		current = strtok(NULL, IGNORE_CHARS);
  		/*following tokens are arguments*/
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    {	
    	int num_args = 0;
    	char name[20];/*  1.char* 2.="" 
    	I have trid all ways, and only this way works well.*/
    	current = strtok(buf, IGNORE_CHARS);
    	/* Next, use strtok() to scan for next character. 
    	If there's nothing, go to the next line */
    	strcpy(name, current);
    	/*deep copy the current token to instruction name*/
    	current = strtok(NULL, IGNORE_CHARS);
    	while(current != NULL)
    	{
    		/*following tokens are arguments*/
    	...
    	address = line*4;
    	/*the offset should be 4 times of line number*/
    	if (translate_inst(output, name, args, num_args, address, symtbl, reltbl) == -1)
    	{
    ...
    /*if flag != 0, some errors occurred*/
    return flag;
}

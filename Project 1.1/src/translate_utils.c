...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    /*if the end pointer is not the end of the string, wrong!*/
    else
    {
    	if (result>=lower_bound && result<=upper_bound)
    	{
			*output = result;
      return 0;
      /*if is in the right interval*/
    	}
    	else
    	{
        /*overflow or underflow, wrong!*/
    		return -1;
    	}
    }
}
...
int translate_reg(const char* str) {
    ...
    /*according to the mips green sheet, get the number*/
    ...
    /*the register is not included, wrong!*/
    else                                return -1;
}
